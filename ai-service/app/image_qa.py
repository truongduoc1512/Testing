...
Chuẩn hóa kiểm duyệt ảnh sản phẩm E-commerce (Shopee/Lazada/Amazon standard):
  ...
  2. Center-Weighted Salient Product Detection: LAB Color Saliency & Contour
...
VALID_PRODUCT_CLASSES = frozenset(
    {
        "product_item",
        "backpack",
        "handbag",
...
@lru_cache(maxsize=1)
def get_model():
    """Chỉ tải mô hình YOLOv8n khi được sử dụng lần đầu."""
...
def compute_salient_product_mask(image_bgr):  # pylint: disable=too-many-locals
    """
    ...
    # 4. Áp dụng bản đồ trọng số trung tâm
    ...
    # 5. Phân ngưỡng để tìm mặt nạ sản phẩm
    _, mask = cv2.threshold(weighted_saliency, 0, 255, cv2.THRESH_BINARY + cv2.THRESH_OTSU)

    # Làm sạch mặt nạ bằng các phép toán hình thái học
    kernel = cv2.getStructuringElement(cv2.MORPH_ELLIPSE, (5, 5))
    mask = cv2.morphologyEx(mask, cv2.MORPH_CLOSE, kernel)
    mask = cv2.morphologyEx(mask, cv2.MORPH_OPEN, kernel)

    # 6. Lọc đường viền sản phẩm
...
def detect_objects(image_bgr):  # pylint: disable=too-many-locals
    """
    ...
    for result in results:
        ...
        for box in boxes:
            ...
            if ratio <= 0.85 and cls_name in VALID_PRODUCT_CLASSES:
                yolo_max_ratio = max(yolo_max_ratio, ratio)
                if cls_name not in detected_classes:
                    detected_classes.append(cls_name)

    # ── 2. Phát hiện sản phẩm nổi bật ở tiền cảnh ──
...
def analyze_image(image_bytes, filename="unknown"):
    ...
    # ── 2. Phát hiện sản phẩm ──
    obj_result = detect_objects(image_bgr)
    has_sufficient_object = (
        obj_result["has_object"] and obj_result["max_object_ratio"] >= MIN_OBJECT_RATIO
    )

    # ── 3. Kiểm tra độ lộn xộn của phông nền bên ngoài sản phẩm ──
    ...
    if is_blurry:
        reasons.append(
            f"Ảnh bị mờ (blur_score={blur_score:.1f} < ngưỡng={BLUR_THRESHOLD:.1f}). "
            "Vui lòng chụp lại với ánh sáng tốt hơn."
        )

    if not obj_result["has_object"]:
        reasons.append(
            "Không phát hiện được đối tượng sản phẩm trong ảnh. "
            "Vui lòng đảm bảo sản phẩm nằm rõ ở trung tâm khung hình."
        )
    elif not has_sufficient_object:
        reasons.append(
            f'Sản phẩm quá nhỏ (chiếm {obj_result["max_object_ratio"] * 100:.1f}% '
            f"< yêu cầu {MIN_OBJECT_RATIO * 100:.0f}%). Vui lòng chụp cận hơn."
        )

    if is_cluttered:
        reasons.append(
            f"Phông nền quá lộn xộn (clutter_score={clutter_score:.2f} > "
            f"ngưỡng={MAX_CLUTTER_THRESHOLD:.2f}). Vui lòng chọn phông nền đơn giản "
            "hoặc tách nền."
        )
