...
| :-: | :--- | :--- | :--- | :--- | :---: |
| **1** | **TC_VOU_01** | Áp dụng mã voucher % hợp lệ với dữ liệu danh định đầy đủ | • `currentUser`: Đã đăng nhập (`employee1`)<br>• `voucherCode`: `"SALE20"` (Giảm 20%, max 50k, active)<br>• `orderAmount`: `25.000.0k` VNĐ (nom)<br>• `discountPercent`: `20.0%` (nom)<br>• `usedCount`: `25` (nom)<br>• `userUsedCount`: `1` (nom)<br>• `expiryDate`: Còn hiệu lực (> now) | **HTTP 200 OK:** Áp dụng mã thành công, chiết khấu 5.000.000 VNĐ vào giỏ hàng. | **V1, V2, V3, V4, V5, V6, B1, D8, ST1** |
| **2** | **TC_VOU_02** | Áp dụng mã voucher tiền mặt cố định (FIXED) | • `voucherCode`: `"FIXED50"` (Giảm 50.000đ cố định)<br>• `orderAmount`: `1.000.000đ`<br>• `currentUser`: `employee1` | **HTTP 200 OK:** Áp dụng thành công, trừ đúng 50.000đ vào tổng tiền thanh toán. | **V1, V2, B25, D8, ST1** |
| **3** | **TC_VOU_03** | Khách vãng lai (Guest) áp dụng mã voucher thành công | • `currentUser`: `Guest` (Chưa đăng nhập, bỏ qua perUserLimit)<br>• `voucherCode`: `"SALE10"`, `orderAmount`: `1.000.000đ` | **HTTP 200 OK:** Áp dụng thành công cho phiên khách vãng lai, giảm 100.000đ. | **V1, V7, B24, D6, ST1** |
| **4** | **TC_VOU_04** | Áp dụng voucher hợp lệ tại tất cả các cận dưới ($min$) | • `orderAmount`: `500.0k` VNĐ ($min$)<br>• `discountPercent`: `1.0%` ($min$)<br>• `usedCount`: `0` ($min$)<br>• `userUsedCount`: `0` ($min$) | **HTTP 200 OK:** Tiếp nhận thành công tại toàn bộ các ngưỡng cận dưới tối thiểu. | **V2, V3, V4, V5, B3, B9, B15, B21, D8** |
| **5** | **TC_VOU_05** | Áp dụng voucher hợp lệ tại tất cả các cận trên ($max$) | • `orderAmount`: `50.000.0k` VNĐ ($max$)<br>• `discountPercent`: `100.0%` ($max$)<br>• `usedCount`: `49` ($max$)<br>• `userUsedCount`: `1` ($max$) | **HTTP 200 OK:** Tiếp nhận thành công tại toàn bộ các ngưỡng cận trên tối đa. | **V2, V3, V4, V5, B6, B12, B18, B22, D8** |
| **6** | **TC_VOU_06** | Áp dụng voucher hợp lệ tại các kề cận dưới ($min^+$) | • `orderAmount`: `501.0k` VNĐ ($min^+$)<br>• `discountPercent`: `1.1%` ($min^+$)<br>• `usedCount`: `1` ($min^+$) | **HTTP 200 OK:** Áp dụng thành công tại các giá trị kề cận dưới. | **V2, V3, V4, B4, B10, B16, D8** |
| **7** | **TC_VOU_07** | Áp dụng voucher hợp lệ tại các kề cận trên ($max^-$) | • `orderAmount`: `49.999.0k` VNĐ ($max^-$)<br>• `discountPercent`: `99.9%` ($max^-$)<br>• `usedCount`: `48` ($max^-$) | **HTTP 200 OK:** Áp dụng thành công tại các giá trị kề cận trên. | **V2, V3, V4, B5, B11, B17, D8** |
| **8** | **TC_VOU_08** | Thay đổi voucher trong giỏ sang mã khác (Ghi đè mã mới) | • Giỏ hàng đang áp dụng mã `"SALE10"`<br>• Gửi `POST /api/v1/vouchers/apply` với mã mới `"SALE20"` | **HTTP 200 OK:** Hủy mã cũ, áp dụng mã mới và tính toán lại mức giảm chính xác. | **V1, ST2** |
| **9** | **TC_VOU_09** | Hủy/Gỡ mã giảm giá khỏi giỏ hàng thành công | • Giỏ hàng đang có mã voucher đã áp dụng<br>• Khách hàng thực hiện gỡ mã khỏi giỏ hàng | **HTTP 200 OK:** Xóa `voucherCode`, đưa `discountAmount = 0`, khôi phục giá gốc. | **ST5** |
| **10** | **TC_VOU_10** | Báo lỗi khi để trống mã giảm giá ($min^-$) | • `voucherCode`: `""` (rỗng hoặc toàn khoảng trắng) | **HTTP 400 Bad Request:** Thông báo lỗi: *"Vui lòng nhập mã giảm giá!"*. | **X1** |
| **11** | **TC_VOU_11** | Báo lỗi khi mã giảm giá không tồn tại trong CSDL | • `voucherCode`: `"NOT_EXIST_CODE_999"` | **HTTP 400 Bad Request:** *"Mã giảm giá không tồn tại hoặc đã bị vô hiệu hóa!"*. | **X2, D1** |
| **12** | **TC_VOU_12** | Báo lỗi khi mã giảm giá đã bị vô hiệu hóa (`active = false`) | • `voucherCode`: `"INACTIVE_VOUCHER"` (`active = false`) | **HTTP 400 Bad Request:** *"Mã giảm giá không tồn tại hoặc đã bị vô hiệu hóa!"*. | **X3, D2** |
| **13** | **TC_VOU_13** | Báo lỗi khi đơn hàng chưa đạt giá trị tối thiểu ($min^-$) | • `voucherCode`: `"MIN500"` (Ngưỡng đơn tối thiểu 500k)<br>• `orderAmount`: `499.0k` VNĐ ($min^-$) | **HTTP 400 Bad Request:** *"Đơn hàng tối thiểu phải từ 500.000 ₫ để áp dụng mã này!"*. | **X4, B2, D4** |
| **14** | **TC_VOU_14** | Báo lỗi khi đơn hàng vượt trần quy định ($max^+$) | • `voucherCode`: `"SALE10"`<br>• `orderAmount`: `50.001.0k` VNĐ ($max^+$) | **HTTP 400 Bad Request:** *"Giá trị đơn hàng vượt quá ngưỡng xử lý!"*. | **X5, B7** |
| **15** | **TC_VOU_15** | Báo lỗi khi tỷ lệ chiết khấu nhỏ hơn 1% ($min^-$) | • `discountPercent`: `0.9%` ($min^-$) | **HTTP 400 Bad Request:** *"Phần trăm chiết khấu không hợp lệ (< 1%)"*. | **X6, B8** |
| **16** | **TC_VOU_16** | Báo lỗi khi tỷ lệ chiết khấu vượt quá 100% ($max^+$) | • `discountPercent`: `100.1%` ($max^+$) | **HTTP 400 Bad Request:** *"Phần trăm chiết khấu không được vượt quá 100%!"*. | **X7, B13** |
| **17** | **TC_VOU_17** | Báo lỗi khi số lượt dùng chung là số âm ($min^-$) | • `usedCount`: `-1` ($min^-$) | **HTTP 400 Bad Request:** *"Số lượt sử dụng không được là số âm!"*. | **X8, B14** |
| **18** | **TC_VOU_18** | Báo lỗi khi mã đã cạn số lượt toàn hệ thống ($max^+$) | • `voucherCode`: `"SOLDOUT"`<br>• `usedCount`: `50` (Chạm trần `usageLimit = 50`) | **HTTP 400 Bad Request:** *"Mã giảm giá đã hết lượt sử dụng toàn hệ thống!"*. | **X9, B19, D5, ST3** |
| **19** | **TC_VOU_19** | Báo lỗi khi số lượt dùng cá nhân là số âm ($min^-$) | • `userUsedCount`: `-1` ($min^-$) | **HTTP 400 Bad Request:** *"Số lượt sử dụng cá nhân không được là số âm!"*. | **X10, B20** |
| **20** | **TC_VOU_20** | Báo lỗi khi tài khoản đã dùng hết hạn mức cá nhân ($max^+$) | • `voucherCode`: `"USED_UP"`<br>• `userUsedCount`: `2` (Chạm trần `perUserLimit = 2`) | **HTTP 400 Bad Request:** *"Tài khoản của bạn đã dùng hết số lượt cho mã này!"*. | **X11, B23, D7** |
| **21** | **TC_VOU_21** | Báo lỗi khi mã voucher đã hết hạn sử dụng | • `voucherCode`: `"EXPIRED_VOU"` (`expiryDate < now`) | **HTTP 400 Bad Request:** *"Mã giảm giá đã hết hạn sử dụng!"*. | **X12, D3, ST4** |
