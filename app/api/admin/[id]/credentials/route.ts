...
export async function GET(
  ...
  try {
    ...
    const admin = await prisma.adminAccount.findFirst({
      ...
      select: { googlePassword: true, totpSecret: true, has2FA: true, updatedAt: true },
    });

    if (!admin) return NextResponse.json({ error: "Not found" }, { status: 404 });

    return NextResponse.json({
      googlePassword: safeDecrypt(admin.googlePassword),
      totpSecret: safeDecrypt(admin.totpSecret),
      hasPassword: Boolean(admin.googlePassword),
    ...
    apiError("/api/admin/[id]/credentials", e);
    return NextResponse.json(
