...
<head>
    ...
    <style>
        ...
        * {{
        ...
        .qa-box {{
            background: linear-gradient(135deg, rgba(67, 97, 238, 0.1), rgba(17, 24, 39, 0.8));
            border: 1px solid var(--card-border);
            border-radius: 14px;
            padding: 24px;
            margin-top: 24px;
...
<body>
    <div class="container">
        ...
        <div class="content-card">
            ...
            <!-- QA VẤN ĐÁP HỘI ĐỒNG -->
            <div class="qa-box">
                <h4><i class="fas fa-graduation-cap"></i> Gợi ý trả lời Giảng viên khi Vấn đáp:</h4>
                <p>
                    <em>"Thưa Thầy/Cô, trong tài liệu đặc tả <code>{os.path.basename(mod['doc_file'])}</code>, em thiết kế <strong>{total_specified} Test Cases nghiệp vụ cốt lõi</strong> theo tiêu chuẩn ISTQB (phủ hết các quy tắc nghiệp vụ, phân hoạch tương đương và giá trị biên).<br>
                    Khi chuyển sang mã nguồn kiểm thử tự động, để đạt mục tiêu kiểm thử hộp trắng (White-box Coverage) che phủ 100% các nhánh rẽ và các giá trị biên cực đại, em triển khai thêm các <code>@ParameterizedTest</code> với nhiều bộ dữ liệu thử nghiệm, nâng tổng số test thực thi của module lên <strong>{code_invocations} test invocations</strong> (nằm trong tổng số 1.074 tests của toàn hệ thống), tất cả đều <strong>Pass 100%</strong>."</em>
                </p>
            </div>
        </div>
